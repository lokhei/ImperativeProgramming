...
char get(text *t, int i) {
  if ((i <0) || (i > strlen(t ->content))) return -1;
  else return t-> content[i];
}

void set(text *t, int i, char c) {
  if ((i < 0) || (i > strlen(t ->content)));
  else t->content[i] = c;
...
text *slice(text *t, int i, int j) {
  ...
  split[j-i] = '\0';
  text *t2 = newText(split);
  return t2;
}

int find(text *t, text *p) {
  char *word = strstr(t -> content, p -> content);
    if (word == NULL) return -1;
    else{
      int n = word- (t->content);
      return n;
    }
}
